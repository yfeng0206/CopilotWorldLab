...
**Data policy (updated 2026-07-05):** the closed-loop benchmark uses our **own** `FrankaDroidEnv`
with **fixed, saved task bundles** generated by a scripted expert — reproducible and inspectable —
over the paper's **four robot tasks** (Reach / Grasp / Reach-with-object / Pick-Place; arXiv
2506.09985 Table 3) and two objects (a rim-graspable **cup** and a rigid **box**), 50 trials per
(task, object) = 400 bundles. Success is a Euclidean delta within a swept sphere radius `x`.
...
`src/bench/success.py` + `src/bench/thresholds.py` for hidden multi-threshold success, and implements
the paper-faithful `pick_place` (3 sub-goals, 4/10/4), CEM chunking, and a GT-vs-V-JEPA demo. In
build: cup/box objects + distractors, the fixed-bundle generator, and the `--bundles` loader.

Next:
1. Add cup/box (+ distractors) to `franka_build.py`; generate the 300 fixed bundles.
2. Run the **50-trial per (task, object)** benchmark across samples **200 -> 400 -> 800** (reach,
   grasp, pick_place x cup/box) and report precision curves + the sample ablation.
...
Empirical outcome: pending the fixed-bundle rebuild (reach/grasp/pick_place x cup/box, 50 trials
each). The multistage sub-goal option is retained as a labeled ablation; the paper-faithful defaults
are reach = 1 goal, grasp = 1 goal, pick_place = 3 sub-goals on the fixed 4/10/4 schedule.
